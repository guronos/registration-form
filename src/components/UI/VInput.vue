<template>
    <div class="input_wrapper">
        <input
            ...
            @change="validateInputData"
        />
        <input
            v-else-if="behavior === 'dropDown'"
            class="drop-down"
            :class="{ error_border: error }"
            type="text"
            v-model="inputData"
        ...
        <div class="icon" @click.stop>
            <span
                ...
                class="icon__svg-wrapper"
                @click="editVisibleStatus"
            >
                <svg
                    width="21"
                    height="10"
        ...
        <div
            v-if="behavior === 'dropDown' && showDropDown"
            class="drop-down-block"
        >
            <div
                v-for="(item, idx) in dropList"
                class="drop-down-block__item"
                :key="idx"
                @click="selectItemDrop(item)"
            >
                {{ item.name }}
            </div>
        </div>
...
export default {
    ...
    props: {
        ...
        errorText: {
            default: 'Проверьте корректность заполнения поля',
        },
    },
    data() {
        return {
    ...
    methods: {
        validateInputData() {
            if (this.inputData) this.$emit('setInput', this.inputData)
        },
        editVisibleStatus() {
...
.icon__svg-wrapper:hover svg path {
    fill: #11579b;
}
...
.drop-down-block__item:first-child {
    border-radius: 12px 12px 0 0;
}

.drop-down-block__item:last-child {
